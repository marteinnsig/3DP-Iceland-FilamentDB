...
Candidate, pending owner runtime acceptance. Compression now calculates force retention from compatible timed holds; the redundant
Add Relaxation Point action is removed; an empty legacy tab is hidden. Comparable Results now includes grouped retention statistics.
Historical records remain accessible through Saved Stress Relaxation when present. Help and existing Full Verification cover
the new contract; no broader tester authorization is required. Debug/Release, Help/docs and vulnerability gates pass.
Profile `20260910173647-74918792` passes Full Data Verification 448/448 and exact database/business-state recovery.
Canonical release remains v65.0.0; owner acceptance is pending.
