...
Debug/Release and Help/docs pass. Profile `20260910181732-9500d441` passes Full Data Verification 449/449 and exact database/business-state recovery.
Owner runtime acceptance is pending; canonical release remains v65.0.0.
...
- Candidate: adds calculated Force retention % to Compression, using compatible factual constant-displacement holds and the unique
  earliest time as reference. Missing/unreached readings, ambiguous reference times and zero reference forces produce no percentage.
- Reference changes notify dependent rows without rebinding or moving the active editor; accepted shared input handlers are reused.
- Removes Add Relaxation Point and its handler. Saved Stress Relaxation appears only for specimens with historical rows.
- Comparable Results summarizes unrounded per-specimen retention by method, target, displacement, cycle and actual time interval.
  Repeated readings do not inflate independent n; mean, sample SD, CV and range use the existing summary conventions.
- Existing Full Verification owns formula and UI contracts; no new runner scenario or authorization is needed. Help is updated.
- Debug/Release, Help/docs and vulnerability gates pass. Profile `20260910173647-74918792` passes Full Data Verification 448/448 and exact database/business-state recovery.
- Owner runtime acceptance remains pending. Canonical release remains v65.0.0; no commit or publication yet.
