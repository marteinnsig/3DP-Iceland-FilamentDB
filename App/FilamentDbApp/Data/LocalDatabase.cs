...
public sealed partial class LocalDatabase
{
    ...
    private void Initialize()
    {
        ...
        connection.Open();
        _ = int.TryParse(
            ReadSchemaVersion(connection),
            NumberStyles.Integer,
            CultureInfo.InvariantCulture,
            out var schemaVersionBeforeInitialize);
        using var command = connection.CreateCommand();
        command.CommandText = @"
...
DROP TABLE IF EXISTS MaterialsImport;";
        ...
        if (schemaVersionBeforeInitialize < 38)
            BackfillLegacyLandedCostCurrencyMetadata(connection);
        RepairV5302DraftCalculationMarkers(connection);
        RetireLegacyWorkbookTablesIfCanonicalReady(connection);
    }

    private static void RepairV5302DraftCalculationMarkers(SqliteConnection connection)
    {
        using var command = connection.CreateCommand();
        command.CommandText = """
            UPDATE PurchaseOrders
            SET LandedCostCalculationVersion = ''
            WHERE TRIM(COALESCE(LandedCostCalculationVersion, '')) = 'legacy-v1'
              AND TRIM(COALESCE(LandedCostCalculatedAtUtc, '')) = ''
              AND TRIM(COALESCE(CostStatus, '')) = 'Draft'
              AND TRIM(COALESCE(LifecycleStatus, '')) = 'Draft'
              AND TRIM(COALESCE(LandedCostRateSource, '')) <> ''
