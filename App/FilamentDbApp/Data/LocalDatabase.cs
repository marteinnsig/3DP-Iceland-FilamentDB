...
ORDER BY MaterialId;";
    ...
    private static long InsertImportRecord(SqliteConnection connection, SqliteTransaction transaction, WorkbookImportData workbook)
    {
        using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = @"
INSERT INTO Imports (ImportedAtUtc, SourceFileName, SourcePath, SchemaVersion)
VALUES ($importedAtUtc, $sourceFileName, $sourcePath, $schemaVersion);
SELECT last_insert_rowid();";
        command.Parameters.AddWithValue("$importedAtUtc", DateTime.UtcNow.ToString("u"));
        command.Parameters.AddWithValue("$sourceFileName", workbook.SourceFileName);
        command.Parameters.AddWithValue("$sourcePath", workbook.SourcePath);
        command.Parameters.AddWithValue("$schemaVersion", SchemaVersion);
        return (long)(command.ExecuteScalar() ?? 0L);
    }

    private static void InsertMaterials(SqliteConnection connection, SqliteTransaction transaction, DataTable materials)
    {
        foreach (DataRow row in materials.Rows)
        {
            var materialId = GetValue(row, "Material ID");
...
(MaterialId, Manufacturer, ProductLine, MarketingName, BaseMaterial, MaterialCategory, VariantFinish, Reinforcement, Color, DiameterMm, SpoolWeightG, ManufacturerSku, PurchaseDate, ManufacturerWebsite, YouTubeReviewUrl, ThumbnailFilename, Video, Notes, TestedStatus, InTensile, InImpact, InStiffness, SortOrder, SourcePriority, WebsiteDisplayName, MaterialKey)
VALUES
($MaterialId, $Manufacturer, $ProductLine, $MarketingName, $BaseMaterial, $MaterialCategory, $VariantFinish, $Reinforcement, $Color, $DiameterMm, $SpoolWeightG, $ManufacturerSku, $PurchaseDate, $ManufacturerWebsite, $YouTubeReviewUrl, $ThumbnailFilename, $Video, $Notes, $TestedStatus, $InTensile, $InImpact, $InStiffness, $SortOrder, $SourcePriority, $WebsiteDisplayName, $MaterialKey);";

                AddParameter(command, "$MaterialId", materialId);
                AddParameter(command, "$Manufacturer", GetValue(row, "Manufacturer"));
                AddParameter(command, "$ProductLine", GetValue(row, "Product Line"));
                AddParameter(command, "$MarketingName", GetValue(row, "Marketing Name"));
                AddParameter(command, "$BaseMaterial", GetValue(row, "Base Material"));
                AddParameter(command, "$MaterialCategory", GetValue(row, "Material Category"));
                AddParameter(command, "$VariantFinish", GetValue(row, "Variant / Finish"));
                AddParameter(command, "$Reinforcement", GetValue(row, "Reinforcement"));
                AddParameter(command, "$Color", GetValue(row, "Color"));
                AddParameter(command, "$DiameterMm", GetValue(row, "Diameter mm"));
                AddParameter(command, "$SpoolWeightG", GetValue(row, "Spool Weight g"));
                AddParameter(command, "$ManufacturerSku", GetValue(row, "Manufacturer SKU"));
                AddParameter(command, "$PurchaseDate", GetValue(row, "Purchase Date"));
                AddParameter(command, "$ManufacturerWebsite", GetValue(row, "Manufacturer Website"));
                AddParameter(command, "$YouTubeReviewUrl", GetValue(row, "YouTube Review URL"));
                AddParameter(command, "$ThumbnailFilename", GetValue(row, "Thumbnail Filename"));
                AddParameter(command, "$Video", GetValue(row, "Video"));
                AddParameter(command, "$Notes", GetValue(row, "Notes"));
                AddParameter(command, "$TestedStatus", GetValue(row, "Tested Status"));
                AddParameter(command, "$InTensile", GetValue(row, "In Tensile"));
                AddParameter(command, "$InImpact", GetValue(row, "In Impact"));
                AddParameter(command, "$InStiffness", GetValue(row, "In Stiffness"));
                AddParameter(command, "$SortOrder", GetValue(row, "Sort Order"));
                AddParameter(command, "$SourcePriority", GetValue(row, "Source Priority"));
                AddParameter(command, "$WebsiteDisplayName", GetValue(row, "Website Display Name"));
                AddParameter(command, "$MaterialKey", GetValue(row, "Material Key"));
                command.ExecuteNonQuery();
            }

            foreach (DataColumn column in materials.Columns)
            {
                using var attribute = connection.CreateCommand();
                attribute.Transaction = transaction;
                attribute.CommandText = @"
INSERT OR REPLACE INTO MaterialAttributes (MaterialId, FieldName, FieldValue)
VALUES ($materialId, $fieldName, $fieldValue);";
                AddParameter(attribute, "$materialId", materialId);
                AddParameter(attribute, "$fieldName", column.ColumnName);
                AddParameter(attribute, "$fieldValue", row[column]?.ToString() ?? string.Empty);
                attribute.ExecuteNonQuery();
            }
        }
    }

    private static void InsertLookups(SqliteConnection connection, SqliteTransaction transaction, DataTable materials)
    {
        InsertLookup(connection, transaction, materials, "Manufacturer", "Manufacturer");
        InsertLookup(connection, transaction, materials, "BaseMaterial", "Base Material");
        InsertLookup(connection, transaction, materials, "MaterialCategory", "Material Category");
        InsertLookup(connection, transaction, materials, "VariantFinish", "Variant / Finish");
        InsertLookup(connection, transaction, materials, "Reinforcement", "Reinforcement");
        InsertLookup(connection, transaction, materials, "Color", "Color");

        foreach (DataRow row in materials.Rows)
        {
            var manufacturer = GetValue(row, "Manufacturer");
...
VALUES ($name, $website)
ON CONFLICT(Name) DO UPDATE SET Website = COALESCE(NULLIF($website, ''), Website);";
            AddParameter(command, "$name", manufacturer);
            AddParameter(command, "$website", GetValue(row, "Manufacturer Website"));
            command.ExecuteNonQuery();
        }
    }

    private static void InsertLookup(SqliteConnection connection, SqliteTransaction transaction, DataTable materials, string lookupType, string columnName)
    {
        if (!materials.Columns.Contains(columnName)) return;

        foreach (var value in materials.Rows.Cast<DataRow>()
                     .Select(row => row[columnName]?.ToString()?.Trim() ?? string.Empty)
                     .Where(value => !string.IsNullOrWhiteSpace(value))
                     .Distinct(StringComparer.OrdinalIgnoreCase))
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = "INSERT OR IGNORE INTO LookupValues (LookupType, Value) VALUES ($type, $value);";
            AddParameter(command, "$type", lookupType);
            AddParameter(command, "$value", value);
            command.ExecuteNonQuery();
        }
    }

    private static void InsertWorkbookSheets(SqliteConnection connection, SqliteTransaction transaction, long importId, IReadOnlyList<ImportedSheetData> sheets)
    {
        foreach (var sheet in sheets)
        {
            var sheetId = InsertSheet(connection, transaction, importId, sheet);
            var columnIds = InsertSheetColumns(connection, transaction, sheetId, sheet.Table);
            InsertSheetRowsAndCells(connection, transaction, sheetId, columnIds, sheet.Table);
        }
    }

    private static long InsertSheet(SqliteConnection connection, SqliteTransaction transaction, long importId, ImportedSheetData sheet)
    {
        using var command = connection.CreateCommand();
        command.Transaction = transaction;
        command.CommandText = @"
INSERT INTO ExcelSheets (ImportId, SheetName, Purpose, HeaderRow, RowCount, ColumnCount)
VALUES ($importId, $sheetName, $purpose, $headerRow, $rowCount, $columnCount);
SELECT last_insert_rowid();";
        command.Parameters.AddWithValue("$importId", importId);
        AddParameter(command, "$sheetName", sheet.SheetName);
        AddParameter(command, "$purpose", sheet.Purpose);
        command.Parameters.AddWithValue("$headerRow", sheet.HeaderRow);
        command.Parameters.AddWithValue("$rowCount", sheet.RowCount);
        command.Parameters.AddWithValue("$columnCount", sheet.ColumnCount);
        return (long)(command.ExecuteScalar() ?? 0L);
    }

    private static Dictionary<int, long> InsertSheetColumns(SqliteConnection connection, SqliteTransaction transaction, long sheetId, DataTable table)
    {
        var columnIds = new Dictionary<int, long>();
        for (var i = 0; i < table.Columns.Count; i++)
        {
            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
INSERT INTO ExcelSheetColumns (SheetId, ColumnIndex, ColumnName)
VALUES ($sheetId, $columnIndex, $columnName);
SELECT last_insert_rowid();";
            command.Parameters.AddWithValue("$sheetId", sheetId);
            command.Parameters.AddWithValue("$columnIndex", i + 1);
            AddParameter(command, "$columnName", table.Columns[i].ColumnName);
            columnIds[i] = (long)(command.ExecuteScalar() ?? 0L);
        }
        return columnIds;
    }

    private static void InsertSheetRowsAndCells(SqliteConnection connection, SqliteTransaction transaction, long sheetId, Dictionary<int, long> columnIds, DataTable table)
    {
        var materialIdColumn = table.Columns.Contains("Material ID") ? table.Columns["Material ID"] : null;

        for (var rowIndex = 0; rowIndex < table.Rows.Count; rowIndex++)
        {
            var row = table.Rows[rowIndex];
            var materialId = materialIdColumn is null ? string.Empty : row[materialIdColumn]?.ToString() ?? string.Empty;

            using var rowCommand = connection.CreateCommand();
            rowCommand.Transaction = transaction;
            rowCommand.CommandText = @"
INSERT INTO ExcelSheetRows (SheetId, RowIndex, MaterialId)
VALUES ($sheetId, $rowIndex, $materialId);
SELECT last_insert_rowid();";
            rowCommand.Parameters.AddWithValue("$sheetId", sheetId);
            rowCommand.Parameters.AddWithValue("$rowIndex", rowIndex + 1);
            AddParameter(rowCommand, "$materialId", materialId);
            var rowId = (long)(rowCommand.ExecuteScalar() ?? 0L);

            for (var columnIndex = 0; columnIndex < table.Columns.Count; columnIndex++)
            {
                var value = row[columnIndex]?.ToString() ?? string.Empty;
                if (string.IsNullOrWhiteSpace(value)) continue;

                using var cellCommand = connection.CreateCommand();
                cellCommand.Transaction = transaction;
                cellCommand.CommandText = @"
INSERT INTO ExcelSheetCells (RowId, ColumnId, CellValue)
VALUES ($rowId, $columnId, $cellValue);";
                cellCommand.Parameters.AddWithValue("$rowId", rowId);
                cellCommand.Parameters.AddWithValue("$columnId", columnIds[columnIndex]);
                AddParameter(cellCommand, "$cellValue", value);
                cellCommand.ExecuteNonQuery();
            }
        }
    }

    private static void InsertTensileResultsFromWebsiteExport(SqliteConnection connection, SqliteTransaction transaction, ImportedSheetData? websiteExportSheet, ImportedSheetData? tensileMeasurementsSheet)
    {
        if (websiteExportSheet is null) return;
        var table = websiteExportSheet.Table;
        if (!table.Columns.Contains("Material ID")) return;
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            using (var command = connection.CreateCommand())
            {
                command.Transaction = transaction;
                command.CommandText = @"
INSERT OR REPLACE INTO TensileResults
(MaterialId, UprightMpa, FlatMpa, StdDevUpright, StdDevFlat, CvUpright, CvFlat, SamplesUpright, SamplesFlat, ConfidenceUpright, ConfidenceFlat, TestNotes, SourceSheet)
VALUES
($MaterialId, $UprightMpa, $FlatMpa, $StdDevUpright, $StdDevFlat, $CvUpright, $CvFlat, $SamplesUpright, $SamplesFlat, $ConfidenceUpright, $ConfidenceFlat, $TestNotes, $SourceSheet);";

                AddParameter(command, "$MaterialId", materialId);
                AddParameter(command, "$UprightMpa", GetValue(row, "Tensile Upright MPa"));
                AddParameter(command, "$FlatMpa", GetValue(row, "Tensile Flat MPa"));
                AddParameter(command, "$StdDevUpright", GetValue(row, "Tensile Upright SD"));
                AddParameter(command, "$StdDevFlat", GetValue(row, "Tensile Flat SD"));
                AddParameter(command, "$CvUpright", GetValue(row, "Tensile Upright CV %"));
                AddParameter(command, "$CvFlat", GetValue(row, "Tensile Flat CV %"));
                AddParameter(command, "$SamplesUpright", GetValue(row, "Tensile Upright Samples"));
                AddParameter(command, "$SamplesFlat", GetValue(row, "Tensile Flat Samples"));
                AddParameter(command, "$ConfidenceUpright", GetValue(row, "Tensile Upright Confidence"));
                AddParameter(command, "$ConfidenceFlat", GetValue(row, "Tensile Flat Confidence"));
                AddParameter(command, "$TestNotes", string.Empty);
                AddParameter(command, "$SourceSheet", websiteExportSheet.SheetName);
                command.ExecuteNonQuery();
            }
        }

        InsertTensileSamples(connection, transaction, tensileMeasurementsSheet);
    }

    private static void InsertTensileSamples(SqliteConnection connection, SqliteTransaction transaction, ImportedSheetData? tensileSheet)
    {
        if (tensileSheet is null) return;
        var table = tensileSheet.Table;
        if (!table.Columns.Contains("Material ID")) return;
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            InsertTensileSamplesForOrientation(connection, transaction, tensileSheet.SheetName, materialId, row, "Upright");
            InsertTensileSamplesForOrientation(connection, transaction, tensileSheet.SheetName, materialId, row, "Flat");
        }
    }

    private static void InsertTensileSamplesForOrientation(SqliteConnection connection, SqliteTransaction transaction, string sheetName, string materialId, DataRow row, string orientation)
    {
        for (var sampleNumber = 1; sampleNumber <= 10; sampleNumber++)
        {
            var columnName = $"{orientation} {sampleNumber}";
            ...
            command.Transaction = transaction;
            command.CommandText = @"
INSERT OR REPLACE INTO TensileSamples (MaterialId, Orientation, SampleNumber, RawValue, SourceSheet)
VALUES ($materialId, $orientation, $sampleNumber, $rawValue, $sourceSheet);";
            AddParameter(command, "$materialId", materialId);
            AddParameter(command, "$orientation", orientation);
            command.Parameters.AddWithValue("$sampleNumber", sampleNumber);
            AddParameter(command, "$rawValue", rawValue);
            AddParameter(command, "$sourceSheet", sheetName);
            command.ExecuteNonQuery();
        }
    }


    private static void InsertImpactSamples(SqliteConnection connection, SqliteTransaction transaction, ImportedSheetData? impactSheet)
    {
        if (impactSheet is null) return;
        var table = impactSheet.Table;
        if (!table.Columns.Contains("Material ID")) return;
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            InsertImpactSamplesForOrientation(connection, transaction, impactSheet.SheetName, materialId, row, "Upright");
            InsertImpactSamplesForOrientation(connection, transaction, impactSheet.SheetName, materialId, row, "Flat");
        }
    }

    private static void InsertImpactSamplesForOrientation(SqliteConnection connection, SqliteTransaction transaction, string sheetName, string materialId, DataRow row, string orientation)
    {
        for (var sampleNumber = 1; sampleNumber <= 10; sampleNumber++)
        {
            var columnName = $"{orientation} needle % {sampleNumber}";
            var rawValue = GetValue(row, columnName);
            if (string.IsNullOrWhiteSpace(rawValue))
            {
                // Early Excel versions used a shorter imported header in a few transition builds.
                rawValue = GetValue(row, $"{orientation} % {sampleNumber}");
            }
            if (string.IsNullOrWhiteSpace(rawValue)) continue;
            ...
            command.Transaction = transaction;
            command.CommandText = @"
INSERT OR REPLACE INTO ImpactSamples (MaterialId, Orientation, SampleNumber, RawValue, SourceSheet)
VALUES ($materialId, $orientation, $sampleNumber, $rawValue, $sourceSheet);";
            AddParameter(command, "$materialId", materialId);
            AddParameter(command, "$orientation", orientation);
            command.Parameters.AddWithValue("$sampleNumber", sampleNumber);
    ...
    private static void InsertStiffnessMeasurements(SqliteConnection connection, SqliteTransaction transaction, ImportedSheetData? stiffnessSheet)
    {
        if (stiffnessSheet is null) return;
        var table = stiffnessSheet.Table;
        if (!table.Columns.Contains("Material ID")) return;
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            var revolutions = GetValue(row, "Revolutions");
            var degrees = GetValue(row, "Degrees");
            var notes = GetValue(row, "Test Notes");
            if (string.IsNullOrWhiteSpace(revolutions) && string.IsNullOrWhiteSpace(degrees) && string.IsNullOrWhiteSpace(notes)) continue;

            using var command = connection.CreateCommand();
            command.Transaction = transaction;
            command.CommandText = @"
INSERT OR REPLACE INTO StiffnessMeasurements (MaterialId, Revolutions, Degrees, TestNotes, SourceSheet)
VALUES ($materialId, $revolutions, $degrees, $testNotes, $sourceSheet);";
            AddParameter(command, "$materialId", materialId);
            AddParameter(command, "$revolutions", revolutions);
            AddParameter(command, "$degrees", degrees);
            AddParameter(command, "$testNotes", notes);
            AddParameter(command, "$sourceSheet", stiffnessSheet.SheetName);
            command.ExecuteNonQuery();
        }
    }


    private static void InsertWebsiteExportSummaries(SqliteConnection connection, SqliteTransaction transaction, ImportedSheetData? websiteExportSheet)
    {
        if (websiteExportSheet is null) return;
        var table = websiteExportSheet.Table;
        if (!table.Columns.Contains("Material ID")) return;
        ...
        foreach (DataRow row in table.Rows)
        {
            ...
            {
                if (IsDescriptorColumn(column.ColumnName)) continue;

                var value = row[column]?.ToString()?.Trim() ?? string.Empty;
                if (string.IsNullOrWhiteSpace(value)) continue;

                var (testType, unit) = ClassifyMetric(column.ColumnName);
                using var command = connection.CreateCommand();
                command.Transaction = transaction;
                command.CommandText = @"
INSERT OR REPLACE INTO TestSummaryValues (MaterialId, TestType, MetricName, MetricValue, Unit, SourceSheet, SourceColumn)
VALUES ($materialId, $testType, $metricName, $metricValue, $unit, $sourceSheet, $sourceColumn);";
                AddParameter(command, "$materialId", materialId);
                AddParameter(command, "$testType", testType);
                AddParameter(command, "$metricName", column.ColumnName);
                AddParameter(command, "$metricValue", value);
                AddParameter(command, "$unit", unit);
                AddParameter(command, "$sourceSheet", websiteExportSheet.SheetName);
                AddParameter(command, "$sourceColumn", column.ColumnName);
                command.ExecuteNonQuery();
            }
        }
    }
...
VALUES ($id,$series,$value,$unit,$status,$baseline,$active,$notes,$measured,$created,$updated);";
    ...
    private static bool IsDescriptorColumn(string columnName)
    {
        return new[]
        {
            "Material ID", "Manufacturer", "Product Line", "Marketing Name", "Base Material", "Material Category",
            "Variant / Finish", "Reinforcement", "Color", "Website Display Name", "YouTube Review URL"
        }.Any(x => string.Equals(x, columnName, StringComparison.OrdinalIgnoreCase));
    }

    private static (string TestType, string Unit) ClassifyMetric(string metricName)
    {
        if (metricName.Contains("Tensile", StringComparison.OrdinalIgnoreCase)) return ("Tensile", metricName.Contains("MPa", StringComparison.OrdinalIgnoreCase) ? "MPa" : string.Empty);
        if (metricName.Contains("Impact", StringComparison.OrdinalIgnoreCase)) return ("Impact", metricName.Contains("kJ", StringComparison.OrdinalIgnoreCase) ? "kJ/m2" : string.Empty);
        if (metricName.Contains("Stiffness", StringComparison.OrdinalIgnoreCase)) return ("Stiffness", metricName.Contains("MPa", StringComparison.OrdinalIgnoreCase) ? "MPa" : string.Empty);
        if (metricName.Contains("Rating", StringComparison.OrdinalIgnoreCase)) return ("Rating", string.Empty);
    ...
    private static (string? FileName, string? ImportedAtUtc) GetLatestImport(SqliteConnection connection)
    {
        using var command = connection.CreateCommand();
        command.CommandText = "SELECT SourceFileName, ImportedAtUtc FROM Imports ORDER BY ImportId DESC LIMIT 1;";
        using var reader = command.ExecuteReader();
        if (!reader.Read()) return (null, null);
        return (reader["SourceFileName"]?.ToString(), reader["ImportedAtUtc"]?.ToString());
    }
