...
public sealed class PublicMaterialSummaryReportPublishingService
{
    ...
    public static IReadOnlyList<string> PublicFieldAllowlist { get; } = new[]
    {
        ...
        "ManufacturerDistribution", "Materials", "Module", "Label", "MaterialId", "MaterialName", "FlexibleResults",
    ...
    private static string BuildHtmlCore(PublicMaterialSummaryReportModel model, DateTime at, string version, string release)
    {
        ...
        const string materialColumns = "<colgroup><col style=\"width:7%\"><col style=\"width:16%\"><col style=\"width:11%\"><col style=\"width:5%\"><col style=\"width:7%\"><col style=\"width:7%\"><col span=\"4\" style=\"width:5%\"><col span=\"2\" style=\"width:7%\"><col style=\"width:13%\"></colgroup>";
        const string materialHeader = "<thead><tr><th>MaterialID</th><th>Material</th><th>Manufacturer</th><th>Type</th><th>Rigid test coverage</th><th>Engineering axes</th><th>Overall</th><th>Tensile</th><th>Impact</th><th>Stiffness</th><th>Consistency</th><th>Layer adhesion</th><th>Reports</th></tr></thead>";
        var materialTables = string.Join("", model.Materials.Chunk(20).Select(chunk =>
        {
            var rows = string.Join("", chunk.Select(row =>
            ...
            return $"<table class=\"material-table\">{materialColumns}{materialHeader}<tbody>{rows}</tbody></table>";
        }));

        return $"<!doctype html><html><head><meta charset=\"utf-8\"><meta name=\"viewport\" content=\"width=device-width,initial-scale=1\"><title>3DPIceland Public Material Summary</title><style>body{{font-family:Segoe UI,Arial;margin:0;background:#f1f5f9;color:#0f172a}}main{{max-width:1380px;margin:30px auto;background:#fff;padding:30px;border-radius:18px}}header{{display:grid;grid-template-columns:1fr auto;gap:20px;border-bottom:3px solid #0f172a}}header img{{width:180px}}.cards{{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;margin:20px 0}}.card,.note,.chart{{border:1px solid #cbd5e1;border-radius:12px;padding:14px}}.value{{font-size:25px;font-weight:800}}.charts{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:18px}}.bar{{display:grid;grid-template-columns:minmax(130px,1fr) minmax(130px,2fr) 52px;gap:10px;align-items:center;margin:9px 0}}.track{{height:14px;background:#e2e8f0;border-radius:99px;overflow:hidden}}.fill{{height:100%;background:#0f172a}}.table-wrap{{max-width:100%;overflow-x:auto}}table{{width:100%;border-collapse:collapse;margin:12px 0 24px}}.material-table{{width:100%;min-width:1180px;margin:0;table-layout:fixed}}.material-table th,.material-table td{{white-space:nowrap}}.material-table th:nth-child(2),.material-table td:nth-child(2),.material-table th:nth-child(3),.material-table td:nth-child(3),.material-table th:nth-child(5),.material-table th:nth-child(6),.material-table th:nth-child(12),.material-table th:nth-child(13),.material-table td:nth-child(13){{white-space:normal;overflow-wrap:break-word}}th,td{{padding:8px;border-bottom:1px solid #e2e8f0;text-align:left;white-space:nowrap}}a{{color:#1d4ed8}}@media(max-width:800px){{header,.cards,.charts{{grid-template-columns:1fr}}}}@media print{{body{{background:#fff}}main{{margin:0;padding:0}}.chart{{break-inside:avoid}}}}</style></head><body><main><header><div><strong>Public Material Summary</strong><h1>3DPIceland engineering dataset</h1><p>Canonical identity, public test coverage and high-level verified engineering results.</p></div><div><img src=\"assets/3dp-iceland-labs-logo-pdf.jpg\" alt=\"3DPIceland Labs\"><p>{H(version)} - {H(release)}<br>{at:yyyy-MM-dd HH:mm:ss}</p></div></header><div class=\"note\"><strong>Public-data contract:</strong> this snapshot contains only MaterialIDs explicitly approved for public reports and existing Verified Material Summary / governed score outputs. Missing results remain n/a; no measurement or score is recalculated.</div><div class=\"cards\"><div class=\"card\"><div>Public materials</div><div class=\"value\">{model.PublicMaterials}</div></div><div class=\"card\"><div>Rigid tests complete (3 modules)</div><div class=\"value\">{model.FullyTested}</div></div><div class=\"card\"><div>Rigid tests partial</div><div class=\"value\">{model.PartiallyTested}</div></div><div class=\"card\"><div>No rigid test results</div><div class=\"value\">{model.NoTestResults}</div></div><div class=\"card\"><div>Manufacturers</div><div class=\"value\">{model.Manufacturers}</div></div><div class=\"card\"><div>Material types</div><div class=\"value\">{model.MaterialTypes}</div></div></div><h2>Native test-result coverage</h2><table><thead><tr><th>Engineering module / score</th><th>Materials</th><th>Rigid coverage</th></tr></thead><tbody>{coverageRows}</tbody></table><div class=\"charts\">{Distribution("Material type distribution", model.MaterialTypeDistribution, model.PublicMaterials)}{Distribution("Manufacturer distribution", model.ManufacturerDistribution, model.PublicMaterials)}</div><h2>Materials in public report scope</h2><div class=\"table-wrap\">{materialTables}</div><h2>Methodology and source</h2><div class=\"note\">Verified means an accepted result is available under the platform validation rules. Results are comparative 3DPIceland measurements and do not replace certified manufacturer datasheets or accredited laboratory testing.</div><p><a href=\"https://iskort.is/3dp/index.html#methodology\">Testing methodology</a> &middot; <a href=\"https://iskort.is/3dp/3DPIceland_Labs_Mechanical_Testing_Methodology_v1.0.pdf\">Engineering methodology whitepaper</a> &middot; <a href=\"report.pdf\">Download PDF</a></p>{string.Concat(model.Materials.Where(x => x.FlexibleResults.Count > 0).Select(x => "<h3>" + H(x.MaterialName) + "</h3>" + FlexibleReportEvidenceService.RenderHtml(x.FlexibleResults)))}<footer>Stable public path: {StableDirectory}/</footer></main></body></html>";
    }

    private static string ApplyCanonicalPrintLayout(string html)
    {
        const string printCss = "@media print{body{background:#fff}main{max-width:none;margin:0;padding:0}header{grid-template-columns:1fr auto!important}.cards{grid-template-columns:repeat(3,minmax(0,1fr))!important}.charts{grid-template-columns:repeat(2,minmax(0,1fr))!important}.chart{break-inside:avoid}.table-wrap{overflow:visible;width:100%}.table-wrap table{width:100%;min-width:0;max-width:100%;font-size:7px;table-layout:fixed}.table-wrap thead{display:table-header-group}.table-wrap tr{break-inside:avoid;page-break-inside:avoid}.table-wrap th,.table-wrap td{box-sizing:border-box;padding:3px;white-space:normal;overflow-wrap:anywhere;word-break:break-word}.table-wrap th:nth-child(1),.table-wrap td:nth-child(1){width:7%}.table-wrap th:nth-child(2),.table-wrap td:nth-child(2){width:16%}.table-wrap th:nth-child(3),.table-wrap td:nth-child(3){width:11%}.table-wrap th:nth-child(4),.table-wrap td:nth-child(4){width:5%}.table-wrap th:nth-child(5),.table-wrap td:nth-child(5){width:7%}.table-wrap th:nth-child(6),.table-wrap td:nth-child(6){width:7%}.table-wrap th:nth-child(13),.table-wrap td:nth-child(13){width:13%}}/* 3DP-PUBLIC-PRINT-LAYOUT-v42.8.3; summary width hardening v44.7.15 */";
