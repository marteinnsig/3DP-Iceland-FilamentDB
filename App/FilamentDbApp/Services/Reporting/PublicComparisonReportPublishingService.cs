...
public sealed class PublicComparisonReportPublishingService
{
    ...
    public static IReadOnlyList<string> PublicFieldAllowlist { get; } = new[]
    {
        ...
        "MaterialId", "MaterialName", "FlexibleResults", "Manufacturer", "ProductLine", "Reinforcement", "TestCoverage",
        "VerifiedEngineeringAxes", "OverallScore", "TensileScore", "ImpactScore",
        "StiffnessScore", "ConsistencyScore", "LayerAdhesionScore", "ThermalScore",
        "ThermalResultTemperatureC", "ThermalMethodVersion", "ThermalLimitation", "MsrpUsdPerKg"
    };
    ...
    {
        return PublicReportScreenThemeService.Apply(ApplyCanonicalPrintLayout(ApplyThermalComparisonContract(BuildHtmlCore(model, generatedAt, versionLabel, releaseTitle, relativeDirectory))));
    }
    ...
    {
        var axes = new (string Name, Func<PublicComparisonMaterialModel, string> Score)[] { ("Overall", x => x.OverallScore), ("Tensile", x => x.TensileScore), ("Impact", x => x.ImpactScore), ("Stiffness", x => x.StiffnessScore), ("Consistency", x => x.ConsistencyScore), ("Layer adhesion", x => x.LayerAdhesionScore), ("Thermal", x => x.ThermalScore) };
    ...
    private static string ApplyThermalComparisonContract(string html)
    {
        const string legacyScoreHeaders = "<th>Consistency</th><th>Layer adhesion</th><th>MSRP USD/kg</th>";
        const string thermalScoreHeaders = "<th>Consistency</th><th>Layer adhesion</th><th>Thermal</th><th>Fixture °C</th><th>Method</th><th>Thermal limitation</th><th>MSRP USD/kg</th>";

        if (!html.Contains("All 5 axes available", StringComparison.Ordinal) ||
            !html.Contains(legacyScoreHeaders, StringComparison.Ordinal))
        {
            throw new InvalidOperationException("The canonical comparison-report template no longer matches the governed thermal publication contract.");
        }

        return html
            .Replace("All 5 axes available", "All 6 axes available", StringComparison.Ordinal)
            .Replace(legacyScoreHeaders, thermalScoreHeaders, StringComparison.Ordinal);
    }
