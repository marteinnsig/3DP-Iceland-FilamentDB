...
(()=>{
  ...
  function drawScore(svg,series,cfg){
    const w=Math.max(480,svg.clientWidth||700),left=Math.min(360,w*.4),right=65,top=30,bottom=40;
    const [label,key,color]=cfg.series[0],limit=Math.max(18,Math.floor((left-20)/6));
    const rows=series.runs.map(run=>{const lines=[];let line='';for(const word of String(run.label||'').split(/\s+/)){for(let start=0;start<word.length;start+=limit){const part=word.slice(start,start+limit);if((line+' '+part).trim().length>limit&&line){lines.push(line);line='';}line=(line+' '+part).trim();}}if(line)lines.push(line);return {run,lines,height:Math.max(40,lines.length*14+12)};});
    const h=Math.max(160,top+bottom+rows.reduce((sum,row)=>sum+row.height,0));svg.setAttribute('viewBox',`0 0 ${w} ${h}`);svg.style.height=h+'px';svg.setAttribute('aria-label','Horizontal experimental score chart');
    const values=rows.map(row=>row.run[key]).filter(Number.isFinite);if(!values.length){const text=el('text',{x:25,y:55,fill:colors.text});text.textContent='No calculated results available.';svg.append(text);return;}
    const max=Math.max(100,...values)*1.08,min=Math.min(0,...values),x=value=>left+(value-min)/(max-min)*(w-left-right),best=Math.max(...values);
    for(let i=0;i<=4;i++){const value=min+(max-min)*i/4,xx=x(value);svg.append(el('line',{x1:xx,x2:xx,y1:top,y2:h-bottom,stroke:colors.grid,'stroke-width':1}));const tick=el('text',{x:xx,y:h-bottom+20,'text-anchor':'middle',fill:colors.text,'font-size':10});tick.textContent=value.toFixed(0);svg.append(tick);}
    let offset=top;rows.forEach(({run,lines,height})=>{const center=offset+height/2,text=el('text',{x:left-12,y:center-(lines.length-1)*7+4,'text-anchor':'end',fill:colors.text,'font-size':11});lines.forEach((line,index)=>{const span=el('tspan',{x:left-12,dy:index?14:0});span.textContent=line;text.append(span);});svg.append(text);const value=run[key];if(Number.isFinite(value)){const description=`${run.label} · ${label}: ${value.toFixed(2)} ${cfg.unit}${run.rank>0?' · Rank #'+run.rank:''}${run.isBaseline?' · Baseline':''}`;const bar=el('rect',{x:Math.min(x(0),x(value)),y:center-11,width:Math.max(2,Math.abs(x(value)-x(0))),height:22,rx:4,fill:value===best?colors.gold:color,stroke:run.isBaseline?colors.blue:'none','stroke-width':run.isBaseline?3:0,tabindex:0,'aria-label':description});const title=el('title');title.textContent=description;bar.append(title);svg.append(bar);const mean=el('text',{x:x(value)+(value<0?-6:6),y:center+4,'text-anchor':value<0?'end':'start',fill:colors.text,'font-size':11});mean.textContent=value.toFixed(2);svg.append(mean);}offset+=height;});
    const axis=el('text',{x:left,y:16,fill:colors.text,'font-size':11});axis.textContent=label+' ('+cfg.unit+')';svg.append(axis);
  }
  function draw(svg,series,kind){
    const cfg=configs[kind]; if(!cfg)return; svg.replaceChildren();
    if(cfg.bar){drawScore(svg,series,cfg);return;}
    ...
    const max=Math.max(0,...values)*1.08||1,min=Math.min(0,...values),range=max-min||1;
